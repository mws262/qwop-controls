    ...
    for idx in range(len(layer_sizes) - 1):
        if idx == len(layer_sizes) - 1:
            current_tensor = nn_layer(current_tensor, layer_sizes[idx], layer_sizes[idx + 1],
                                      name_prefix + str(idx),
                                      act=final_activation)  # Note: removed last activation. This needs to be done outside this function. It was problematic with softmax activation since the loss function wants unscaled logits rather than softmaxed values.
        else:
            current_tensor = nn_layer(current_tensor, layer_sizes[idx], layer_sizes[idx + 1],
...
if __name__ == "__main__":
    ...
    parser.add_argument('-s', '--savepath', type=str,
                        help='<Required> File path and name to save the network graph to.',
                        required=True)
    ...
    parser.add_argument('-w', '--weightstd', type=float,
                        help='<Optional> Standard deviation initialization for weights. (default = 0.1).',
                        required=False,
                        default=0.1)
    ...
    parser.add_argument('-a', '--activations', type=str,
                        help='<Optional> Nonlinear activation type (default leaky_relu).',
                        required=False, default="leaky_relu",
                        choices=['relu', 'leaky_relu', 'elu', 'sigmoid', 'tanh', 'identity'])
    parser.add_argument('-ao', '--activationsout', type=str, help='<Optional> Output layer activation.',
                        required=False, default="identity",
                        choices=['relu', 'leaky_relu', 'elu', 'sigmoid', 'tanh', 'identity', 'softmax'])
    parser.add_argument('-ls', '--loss', type=str, help='<Optional> Loss type.',
                        required=False, default="huber",
                        choices=['huber', 'mse', 'cross_entropy', 'policy_gradient', 'qlearn'])
