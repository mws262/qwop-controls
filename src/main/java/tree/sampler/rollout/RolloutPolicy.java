...
public abstract class RolloutPolicy {
    ...
    static IActionGenerator rolloutActionGenerator = getRolloutActionGenerator();
...
//        actionQueue.clearAll();
    ...
    NodeQWOPExplorableBase<?> randomRollout(NodeQWOPExplorableBase<?> startNode, IGameInternal game, int maxTimesteps) {
        ...
        // If we use the actions generated by startNode, they will be from the action generator of the full tree, not
        // the rollout action generator. That's why we explicitly need to get an action from the rolloutActionGenerator.
        Action childAction = rolloutActionGenerator.getPotentialChildActionSet(startNode).getRandom();
        ...
        while (!rolloutNode.getState().isFailed() && timestepCounter < maxTimesteps) {
            ...
            if (!rolloutNode.getState().isFailed()) {
                childAction = rolloutNode.getUntriedActionRandom();
            }
        }
        return rolloutNode;
    }
