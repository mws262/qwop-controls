 ...
 *
 */
public abstract class NodeExplorable extends NodeQWOP {
    ...
    public NodeExplorable(State rootState, IActionGenerator actionGenerator) {
        super(rootState);
        this.actionGenerator = actionGenerator;
        untriedActions = actionGenerator.getPotentialChildActionSet(this);
    }

    public NodeExplorable(State rootState) {
        super(rootState);
        this.actionGenerator = new ActionGenerator_Null();
    }

    public NodeExplorable(NodeQWOP parent, Action action, State state, IActionGenerator actionGenerator) {
    ...
    public void propagateFullyExploredStatus_lite() {
        ...
        if (!getState().isFailed()) {
            ...
            for (NodeQWOP child : getChildren()) {
                if (child instanceof NodeExplorable && !((NodeExplorable) child).isFullyExplored()) {
                    flag = false;
                }
            }
        }
        ...
        if (getTreeDepth() > 0) { // We already know this node is fully explored, check the parent.
            NodeQWOP parent = getParent();
            if (parent instanceof NodeExplorable) {
                ((NodeExplorable) parent).propagateFullyExploredStatus_lite();
            }
        }
    }
    ...
    private void propagateFullyExplored_complete() {
        ArrayList<NodeQWOP> leaves = new ArrayList<>(5000);
        getLeaves(leaves);

        // Reset all existing exploration flags out there.
        for (NodeQWOP leaf : leaves) {
            NodeQWOP currNode = leaf;
            while (currNode.getTreeDepth() > getTreeDepth()) {
                if (currNode instanceof NodeExplorable)
                    ((NodeExplorable) currNode).setFullyExploredStatus(false);
        ...
        for (NodeQWOP leaf : leaves) {
            if (leaf instanceof NodeExplorable) {
                ((NodeExplorable) leaf).propagateFullyExploredStatus_lite();
            }
        }
    }
