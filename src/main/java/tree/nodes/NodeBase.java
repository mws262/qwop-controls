...
abstract class NodeBase {
     ...
     */
    private final NodeBase parent;

    /**
     * Child nodes. Not fixed size any more.
     */
    private final List<NodeBase> children = new CopyOnWriteArrayList<>();
    ...
    public NodeBase(NodeBase parent) {
        ...
        NodeBase currentNode = getParent();
     ...
     */
    public void removeFromChildren(NodeBase node) {
     ...
     */
    public NodeBase getChildByIndex(int childIndex) {
     ...
     */
    public NodeBase getRandomChild() {
        if (children.isEmpty())
     ...
     */
    public Collection<NodeBase> getNodesBelow(Collection<NodeBase> nodeList) {
     ...
     * @return Returns the list of nodes below. This is done in place, so the object is the same as the argument one.
     */
    public Collection<NodeBase> getLeaves(Collection<NodeBase> leaves) {
     ...
     */
    public void recurseDownTreeInclusive(Consumer<NodeBase> operation) {
    ...
    public void recurseDownTreeExclusive(Consumer<NodeBase> operation) {
        for (NodeBase child : children) {
    ...
    public void recurseUpTreeInclusive(Consumer<NodeBase> operation) {
        operation.accept(this);
        if (treeDepth > 0) {
            parent.recurseUpTreeInclusive(operation);
        }
    }
    public void recurseUpTreeInclusiveNoRoot(Consumer<NodeBase> operation) {
        operation.accept(this);
        if (treeDepth > 1) {
            parent.recurseUpTreeInclusive(operation);
        }
    ...
    public void applyToLeaves(Consumer<NodeBase> operation) {
