...
// Combined with testing for StateQWOPDelayEmbedded
public class GameQWOPCachingTest {
    ...
    private void checkSpecificDelayEmbedding(int delay, int numDelayedStates) {
        float tol = 1e-6f;
        GameQWOPCaching gameCache = new GameQWOPCaching(delay, numDelayedStates, GameQWOPCaching.StateType.POSES);
        Assert.assertEquals((numDelayedStates + 1) * 36, gameCache.getStateDimension());
        GameQWOP gameBasic = new GameQWOP();

        StateQWOP initState = (StateQWOP) GameQWOP.getInitialState();
        ...
        Assert.assertEquals((numDelayedStates + 1) * 36, s.getStateSize());
    ...
    public void finiteDifferenceStates() {
        ...
        StateQWOP s3 = new StateQWOP(f3, false);
        StateQWOP s4 = new StateQWOP(f4, false);
        StateQWOP s5 = new StateQWOP(f5, false);

        StateQWOPDelayEmbedded fullState = new StateQWOPDelayEmbedded_HigherDifferences(new StateQWOP[]{s1, s2, s3, s4, s5});
...
//            stdev1[i] = i;
//        }
//        StateQWOP stdevState1 = new StateQWOP(stdev1, false);
//
//        float[] mean1 = new float[72];
//        for (int i = 0; i < 72; i++) {
//            mean1[i] = i;
//        }
//        StateQWOP meanState1 = new StateQWOP(mean1, false);
//
//        StateNormalizerQWOP.StateStatistics stateStats = mock(StateNormalizerQWOP.StateStatistics.class);
//        when(stateStats.getStdev()).thenReturn(stdevState1);
//        when(stateStats.getMean()).thenReturn(meanState1);
//
//        GameQWOPCaching gameCache = new GameQWOPCaching(1, 3, GameQWOPCaching.StateType.POSES);
//        IState st = gameCache.getCurrentState();
//        IState init = GameQWOP.getInitialState();
