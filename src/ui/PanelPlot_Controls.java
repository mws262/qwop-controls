...
public class PanelPlot_Controls extends PanelPlot implements KeyListener {
	...
	public void update(Node plotNode) {
		...
		List<State> statesBelow = nodes.stream().map(n -> n.state).collect(Collectors.toList()); // Convert from node list to state list.
	...
	public void changePlots() {
		...
		while (it.hasNext()) {
			...
			Float[] yData = nodes.stream().map( n -> (float)n.getActionTimesteps()).toArray(Float[] :: new);
