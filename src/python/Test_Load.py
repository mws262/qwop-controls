...
# Make correctly sized placeholders for inputs and outputs of the net
def placeholder_inputs(batch_size):
    ...
    Returns:
      state_placeholder: state vars placeholder.
      labels_placeholder: Labels placeholder.
      """
    state_placeholder = tf.placeholder(tf.float32, shape=(batch_size, stateVecLength))
    labels_placeholder = tf.placeholder(tf.int32, shape=(batch_size,2))
    return state_placeholder, labels_placeholder

# Put data into the dict (hashmap) to be fed into the placeholders by tensorflow
def fill_feed_dict(data_set, state_pl, labels_pl):
  ...
  Args:
    ...
    state_pl: The images placeholder, from placeholder_inputs().
  ...
  # `batch size` examples.
  state_feed, labels_feed = data_set.next_batch(FLAGS.batch_size, FLAGS.fake_data)
  feed_dict = {
      state_pl: state_feed,
...
def do_eval(sess, eval_correct, state_placeholder, labels_placeholder, data_set):
  ...
  for step in xrange(steps_per_epoch):
    feed_dict = fill_feed_dict(data_set,state_placeholder,labels_placeholder)
